...
void wypelnij_odczyt(Odczyt * odczyt, LSM9DS1 * imu1, LSM9DS1 * imu2)
{
...
void wypisz_odczyt(FILE *fp, Odczyt * odczyt, int csv)
{
    if(!csv)
    {
        ...
        fprintf(fp, "%s + %ldus,", repr, odczyt->czas.tv_usec);
    }
    else{
        fprintf(fp,"%ld.%ld,",odczyt->czas.tv_sec,odczyt->czas.tv_usec);
    }
